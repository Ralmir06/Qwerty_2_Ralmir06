...
public class Perehod {
	...
	String T=line.substring(line.indexOf(Path_V.konez_Varianta)+Path_V.konez_Varianta.length()+1,line.indexOf("s"+Path_V.konez_Varianta)+-1);
	/*String zT="s"+Path_V.konez_Varianta;
	System.out.println(zT);
	int rT=Path_V.konez_Varianta.length()+1;
	String T=line.substring(line.indexOf(Path_V.konez_Varianta),line.indexOf("s"+Path_V.konez_Varianta));
	*/
	System.out.println("Показываю автоматическим номер комнаты БЕЗ ОБРАБОТКИ в число: "+T);
	Integer.parseInt(T);
	System.out.println("Показываю автоматический номер комнаты: "+T);// Некоторые особые символы он не читает. Используй буквенные сочетания!
	
	System.out.println("Показываю вариант ведущий в комнату: "+Path_V.TOv);
	System.out.println();
		
	Perem.T=Integer.parseInt(T); // Perem.T будет РАВЕН значению из подстроки от Конца варианта до Символа стопора, из файла Комнаты.
	try {
		Path_T.path_T();
